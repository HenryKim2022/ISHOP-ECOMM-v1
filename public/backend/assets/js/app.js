...
jQuery(function ($) {
    ...
    $('.searchNav').on('keyup', function () {
        ...
        var appendedItems = []; // Array to store already appended item texts
    
        searchResult.each(function (index, item) {
            var item_url = $(item).attr('href');
            var item_text = $(item).text().replace(/(\d+)/g, '').trim();
    
            // Check if item_text already exists in the appendedItems array
            if (appendedItems.indexOf(item_text) === -1) {
                $(result).append(`
                    <li>
                        <a href="${item_url}" class="d-block">${item_text}</a>
                    </li>
                `);
                appendedItems.push(item_text); // Add item_text to the appendedItems array
            }
        });
    });
